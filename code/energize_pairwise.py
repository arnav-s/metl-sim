...
def prep_working_dir(template_dir, working_dir, pdb_fn, chain, variant,
    ...
    assert "relax.xml" not in files_to_copy, \
        "Cannot name a file in template directory relax.xml - protected name , use relax_temp.xml instead for the template relax xml script"
    assert "flags_relax" not in files_to_copy , \
        "Cannot name a file in template directory flags_relax - protected name, us flags_relax_temp instead for the template relax flags "
